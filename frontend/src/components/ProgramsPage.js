      ...
      <div className="p-6">
        ...
        {groepen.length > 0 && (
          <div className={`${color.bg} ${color.border} border rounded-lg p-4 mb-4`}>
            ...
            <div className="space-y-2">
              {groepen.filter(g => g.status === 'Beschikbaar' || g.status === 'In planning').slice(0, 2).map((groep, idx) => (
                <div key={idx} className="flex justify-between items-center text-sm">
                  <div>
                    <span className="font-medium">Groep {groep.groepnummer}</span>
                    <div className="text-xs text-gray-600">📍 {groep.gli_aanbieder}</div>
                  </div>
              ...
              {groepen.length > 2 && (
                <p className="text-xs text-gray-600 mt-2">
                  +{groepen.length - 2} meer beschikbaar
                </p>
              )}
            </div>
          </div>
