...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('gli_token');
    if (token) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // Token is invalid
      localStorage.removeItem('gli_token');
  ...
  const login = async (email, password) => {
    try {
      ...
      localStorage.setItem('gli_token', token);
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      setUser(userData);
        ...
        <div className="min-h-screen bg-gray-50">
          ...
          <main>
            <Routes>
              ...
              <Route path="/gli-aanbieders" element={<GLIAanbiedersPage />} />
              <Route path="/verwijzerstool" element={<VerwijzersToolPage />} />
